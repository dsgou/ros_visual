 ...
 * 
 * PARAMETERS:
 * 			-the pointer that contains the depth image 
 * 			 and its metadata
 * RETURN --
 */
 ...
 * RETURN --
 */
void Depth_processing::track(vector< Rect_<int> >& cur_boxes, People& collection, int rank, float threshold)
{
	bool exists = false;
	float step = 1.2;
 ...
 * @return -
 */
void Depth_processing::estimateForeground(Mat& cur_Mat, Mat& back_Mat, Mat& dst_Mat)
{
	uchar *back, *cur, *dst;
	for(int y = 0; y < 1; y++)
	{
 ...
 * @return -
 */
void Depth_processing::estimateBackground(Mat& src, Mat& dst, vector<Mat>& storage, int recursion, float ratio, int index)
{
 ...
 * RETURN: --
 */
void Depth_processing::grayToDepth(Mat& src, Mat& dst, float max_depth)
{
 ...
 * RETURN: --
 */
void Depth_processing::depthToGray(Mat& src, Mat& dst, float min_depth, float max_depth)
{
 ...
 * RETURN: --
 */
void Depth_processing::frameDif(Mat& src1, Mat& src2, Mat& dst, float threshold)
{
	uchar *src, *temp, *dest;
 ...
 * RETURN: --
 */
void Depth_processing::rectFill(Mat& src, float threshold, int range)
{
 ...
 * 
 */
void Depth_processing::rightHorizontalFill(Mat& cur_Mat, float threshold, bool scale)
{
	uchar *cur;
	int channels = cur_Mat.channels();
	int cols = cur_Mat.cols;
 ...
 * 
 */
void Depth_processing::upVerticalFill(Mat& src, float threshold, bool flag)
{
	uchar *cur;
	int channels = src.channels();
	int cols = src.cols;
 ...
 * 
 */
void Depth_processing::upVerticalFill2(Mat& src, float threshold, bool scale)
{
	uchar *cur;
	int channels = src.channels();
	int cols = src.cols;
 ...
 * 
 */
void Depth_processing::calculatePosition(Rect_<int>& rect, Depth_processing::Position& pos)
{
 ...
 * 
 */
double Depth_processing::calculateDepth(const Mat& src)
{
	...
	if(cols > 0 && rows > 0)
	{
		...
		{	
			...
			/*
			depthToGray(src, src, 0, max_depth);
			for(int i = 0; i < src.rows; i++)
			{
				uchar* cur = src.ptr<uchar>(i);	
				for(int j = 0; j < src.cols; j++)
				{
					if(rep.cidx[i*src.cols + j] == index[0])
					{
						cur[j] = 0;
					}
					else
 ...
 * 
 */
double Depth_processing::minDepth(vector<double> vec, int number)
{
	double result = 0;
	double min = 0;
	int done = 0;
 ...
 * 
 */
double Depth_processing::centerDepth(const Mat& src, int number)
{
 ...
 * 
 */
kmeansreport Depth_processing::clusterize(const vector<float>& vec, int clusters)
{
	clusterizerstate s;
	kmeansreport rep;
 ...
 */
double Depth_processing::combineDepth(double saveMin, double saveCenter, double saveCluster, double min_depth, double max_depth)
{
	double result;
